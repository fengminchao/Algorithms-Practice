...
public class MinNumberInRotateArray {
    ...
    public static void main(String[] args){
        ...
        int r = new MinNumberInRotateArray().minNumberInRotateArray(arr);
    ...
    private int findMinNum(int[] array, int start, int end) {
        if (start == end) {
            return array[start];
        }
        int mid = (start + end) / 2;
        if (array[mid] < array[end]) {
            return findMinNum(array, start, mid);
        } else {
            return findMinNum(array, mid + 1, end);
        }
    }
}
